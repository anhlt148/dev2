    ...
    dropbox.filedrop({
        ...
        error: function (err, file) {
            switch (err) {
                case 'BrowserNotSupported':
                    showMessage('TrĂ¬nh duyá»‡t cá»§a báº¡n khĂ´ng há»— trá»£ HTML5!');
                    break;
                case 'TooManyFiles':
                    alert('Cáº¥u hĂ¬nh chá»‰ cho phĂ©p 5 áº£nh');
                    break;
                case 'FileTooLarge':
                    alert(file.name + ' kĂ­ch thÆ°á»›c quĂ¡ lá»›n (<2Mb)');
                    break;
                default:
                    break;
            }
        ...
        beforeEach: function (file) {
            if (!file.type.match(/^image\//)) {
                alert('Chá»‰ cho phĂ©p táº£i áº£nh lĂªn server!');
